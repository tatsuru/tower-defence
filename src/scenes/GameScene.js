...
import { TOWER_DEFS, scaledCost, ADVANCED_TOWER_KINDS } from '../data/towers';
...
const CELL_COLORS = {
    ["empty" /* CellType.Empty */]: 0x2d5a27,
    ["path" /* CellType.Path */]: 0x8b6914,
    ["blocked" /* CellType.Blocked */]: 0x3a3a3a,
    ["entrance" /* CellType.Entrance */]: 0x1a7abf,
    ["exit" /* CellType.Exit */]: 0xbf1a1a,
};
export class GameScene extends Phaser.Scene {
    constructor() {
        ...
        Object.defineProperty(this, "waveBanner", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "enemyTooltip", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "towerPanel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "detailPanel", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        });
        Object.defineProperty(this, "prepOverlay", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    create() {
        ...
        this.waveManager = new WaveManager(this, this.state, this.mapData.path, () => { this.onWaveComplete(); }, {
            onWaveStart: (wave) => {
        ...
        this.towerPanel = new TowerPanel(this, this.state, (kind) => this.towers.filter((t) => t.kind === kind).length);
    ...
    onHover(ptr) {
        ...
        const canPlace = isBuildable(gridCell) &&
            kind !== null &&
    ...
    sellTower(tower) {
        ...
        this.mapData.grid[tower.row][tower.col].type = "empty" /* CellType.Empty */;
        tower.destroy();
        this.recalcAllSynergies();
    }
    placeTower(kind, col, row) {
        const gridCell = this.mapData.grid[row][col];
        if (!isBuildable(gridCell))
            return;
        ...
        this.mapData.grid[row][col].type = "blocked" /* CellType.Blocked */;
        this.recalcAllSynergies();
    ...
    showUnlockBanner(name) {
        const text = this.add.text(this.scale.width / 2, this.scale.height / 2 - 60, `解放: ${name}`, { fontSize: '20px', color: '#ffdd44', fontStyle: 'bold', stroke: '#000000', strokeThickness: 3 }).setOrigin(0.5).setDepth(200);
        this.tweens.add({
            targets: text,
            y: text.y - 40,
            alpha: { from: 1, to: 0 },
    ...
    recalcAllSynergies() {
        for (const tower of this.towers) {
            const neighbors = this.towers.filter((t) => t !== tower && Math.abs(t.col - tower.col) <= 1 && Math.abs(t.row - tower.row) <= 1);
    ...
    drawGrid() {
        ...
        this.add.text(GRID_OFFSET_X + (GRID_COLS - 1) * CELL_SIZE + 4, GRID_OFFSET_Y + xRow * CELL_SIZE + 4, 'OUT', { fontSize: '10px', color: '#ffffff', fontStyle: 'bold' });
    }
    restartGame() {
